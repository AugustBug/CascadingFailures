# -*- coding: utf-8 -*-
"""
Created on Tue Apr 29 18:20:13 2019
...
matt = pd.read_csv('datasetBinary.csv', header=None, dtype='float32')
...
dense2 = tf.layers.Dense(units=64, activation=tf.nn.leaky_relu)
dense3 = tf.layers.Dense(units=32, activation=tf.nn.relu)

logits = tf.layers.Dense(units=1, activation=tf.nn.leaky_relu)
...
logit = tf.reshape(output, [-1,nOutputs])
...
costP = tf.Print(cost,[y[:],'                 ', logit[:],'             ',  cost, '    ' ,tf.shape(y)] , '\nprinting cost: ', summarize=50)
...
#optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate, epsilon=0.1,beta1=0.8,beta2=0.2).minimize(loss)
optimizer = tf.train.AdamOptimizer(learning_rate=lb.learning_rate).minimize(totalCost)
'''
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    while step < training_iters and loss_value > target_loss:
        ...
        batch_y = np.reshape(y_train[start:end] , (-1,n_outputs) )
    ...
    batchTest_y = np.reshape(y_test, (-1,nOutputs))
